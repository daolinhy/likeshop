...
export default {
            ...
            .then(() => {
                ...
                uni.$on('payment', (params) => {
                    setTimeout(() => {
                        ...
                        if (params.result) {
                            uni.redirectTo({
                                url: `/bundle2/pages/pay_result/pay_result?id=${params.order_id}`
                            })
                        } else {
                            uni.redirectTo({
                                url: '/bundle2/pages/user_order/user_order'
                            })
                        }
                    }, 500)
                })
    ...
    methods: {
        ...
        onAddressExpress() {
            uni.navigateTo({
                url: `/bundle2/pages/user_address/user_address?type=${1}`
            })
        },
        ...
        async handleOrderSubmit(from) {
            ...
            try {
                ...
                if (code == 1) {
                    uni.redirectTo({
                        url: `/bundle2/pages/payment/payment?from=${data.type}&order_id=${data.order_id}`
                    })
                } else {
                    throw new Error(msg)
                }
